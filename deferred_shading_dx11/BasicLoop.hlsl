...
//--------------------------------------------------------------------------------------
float4 BasicLoop(SurfaceData surface)
{
    ...
    [flatten] if (surface.positionView.z < mCameraNearFar.y) {
        [unroll] for (uint lightIndex = 0; lightIndex < min(MAX_LIGHTS, totalLights); ++lightIndex) {
